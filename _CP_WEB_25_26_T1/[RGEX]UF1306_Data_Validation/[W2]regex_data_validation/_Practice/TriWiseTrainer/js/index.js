...
(function(){
  ...
  async function loadConceptForBank(filename){
    ...
    // Compute a stable base key for both URL and blob cases
   const optLabel = bankSelect?.selectedOptions?.[0]?.textContent || '';
   const baseKey = bankBaseKeyFromPath(filename, optLabel); // e.g., "arrays_showcase" or "filename"
    cxInfo.textContent = 'Bank: ' + filename;
    const key = 'concepts:' + filename;

    const local = loadConceptsLocal(baseKey);
    if (local){
      cxEdit.value = local;
    ...
    try{
      ...
      cxView.innerHTML = renderMd(text);
       ...
       // 3) placeholder if nothing found
  const placeholder = `# ${baseKey}.md\n\nAdd definitions, examples, and quick rules here.\n`;
...
/// INIT HELPER 
const LAST_BANK_KEY = 'tw:lastBank';

function rememberSelectedBank() {
  const v = bankSelect.value;
  if (v && v !== '__browse__' && v !== '') {
    localStorage.setItem(LAST_BANK_KEY, v);
  }
}

function addBrowseSupport() {
  // Append separator + Browse… once
  const hasBrowse = Array.from(bankSelect.options).some(o => o.value === '__browse__');
  if (!hasBrowse) {
    const sep = document.createElement('option');
    sep.value = '';
    sep.textContent = '──────────';
    bankSelect.appendChild(sep);

    const br = document.createElement('option');
    br.value = '__browse__';
    br.textContent = '📂 Browse… (load .jsonl)';
    bankSelect.appendChild(br);
  }

  const picker = document.getElementById('bankPicker');
  if (!picker) return;

  // When user picks the Browse… option, open the file dialog
  bankSelect.addEventListener('change', async () => {
    if (bankSelect.value !== '__browse__') {
      rememberSelectedBank();
      return;
    }
    picker.click();
  });

  // When a file is chosen, make a blob: URL and load it immediately
  picker.addEventListener('change', async () => {
    const file = picker.files?.[0];
    if (!file) {
      // revert to last selection if user canceled
      const last = localStorage.getItem(LAST_BANK_KEY);
      if (last && Array.from(bankSelect.options).some(o => o.value === last)) {
        bankSelect.value = last;
      }
      return;
    }
    const url = URL.createObjectURL(file);
    const label = file.name.replace(/\s+/g, ' ');
    // insert a temporary option so the select shows it
    const opt = document.createElement('option');
    opt.value = url;
    opt.textContent = `Local: ${label}`;
    bankSelect.insertBefore(opt, bankSelect.firstChild);
    bankSelect.value = url;

    await loadSelectedBank();     // load immediately for local file
    rememberSelectedBank();
    // === NEW: offer to pair a .md ===
  const conceptsPicker = document.getElementById('conceptsPicker');
  const wantMd = confirm('Do you want to attach a Concepts .md file for this bank?');
  if (conceptsPicker && wantMd) {
    // one-time handler for the chosen .md
    const once = async () => {
      conceptsPicker.removeEventListener('change', once);
      const md = conceptsPicker.files?.[0];
      if (!md) return;
      const mdText = await md.text();

      // compute stable base key from the select option label
      const optLabel = bankSelect?.selectedOptions?.[0]?.textContent || '';
      const baseKey = bankBaseKeyFromPath(bankSelect.value, optLabel);

      saveConceptsLocal(baseKey, mdText);

      // update Concepts live if UI present
      if (cxEdit && cxView) {
        cxEdit.value = mdText;
        if (typeof renderMd === 'function') cxView.innerHTML = renderMd(mdText);
      }
      alert('Concepts attached for ' + baseKey);
    };
    conceptsPicker.addEventListener('change', once);
    conceptsPicker.click(); // open the .md picker
  }

  // (We still call loadConceptForBank — it will now use the saved local concepts if present)
  if (typeof window.loadConceptForBank === 'function') {
    window.loadConceptForBank(bankSelect.value);
  }

    // We intentionally DO NOT call loadConceptForBank for blob: URLs
    // because there is no sibling .md to fetch by path.
  });
}

// MD helpers for select menu
// --- Concepts pairing helpers ---
function bankBaseKeyFromPath(pathOrUrl, optLabelText){
  try{
    // For blob: URLs (local file), use the option's label if we have it:  "Local: filename.jsonl"
    if (/^blob:/.test(pathOrUrl) && optLabelText) {
      const m = optLabelText.replace(/^Local:\s*/,'').trim(); // "filename.jsonl"
      return m.replace(/\.jsonl$/i,'');                       // "filename"
    }
    // For normal paths (http/.../banks/foo.jsonl)
    const leaf = pathOrUrl.split('/').pop() || pathOrUrl;
    return leaf.replace(/\.jsonl$/i,'');                      // "foo"
  }catch(_){
    return String(pathOrUrl || 'bank').replace(/\.jsonl$/i,'');
  }
}

// Read/save concepts text in localStorage by base key
function loadConceptsLocal(baseKey){
  return localStorage.getItem('concepts:' + baseKey) || null;
}
function saveConceptsLocal(baseKey, mdText){
  localStorage.setItem('concepts:' + baseKey, mdText || '');
}
...
/* ========================================================================== */
(async function init(){
  // Add “Browse…” to the existing select without disturbing your options
  addBrowseSupport();

  // Prefer last used bank if still present in the list
  const last = localStorage.getItem(LAST_BANK_KEY);
  if (last && Array.from(bankSelect.options).some(o => o.value === last)) {
    bankSelect.value = last;
  } else {
    // Your original default-picking logic
  ...
  if (!bankSelect.value) {
    BANK = FALLBACK.slice(); window.BANK = BANK; render(); return;
  }

  // Load the selected bank now
  await loadSelectedBank();
  rememberSelectedBank();

  // Load Concepts only when there is a real path (not a blob URL)
  if (!/^blob:/.test(bankSelect.value) && typeof window.loadConceptForBank === 'function') {
    window.loadConceptForBank(bankSelect.value);
  }
})();
